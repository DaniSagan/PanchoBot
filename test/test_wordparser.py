...
from wordparser import ParserGroup, ParserType, WordParser, GroupQuantityType, MatchResult


class TestParserGroup(unittest.TestCase):
    def test_from_str(self):
        g: ParserGroup = ParserGroup.from_str('name:w')
    ...
    def test_eq(self):
        g1: ParserGroup = ParserGroup.from_str('p1:w')
        g2: ParserGroup = ParserGroup.from_str('p1:w')
        g3: ParserGroup = ParserGroup.from_str('p2:w')
        g4: ParserGroup = ParserGroup.from_str('p2:n')
...
class TestWordParser(unittest.TestCase):
    def test_from_str(self):
        wp: WordParser = WordParser.from_str('p1:w p2:2n p3:*w')
    ...
    def test_match_1_w(self):
        wp: WordParser = WordParser.from_str('p1:w')
        res: MatchResult = wp.match('hello')
        self.assertTrue(res.success)
        self.assertEqual(res.results['p1'], 'hello')

    def test_match_2_w(self):
        wp: WordParser = WordParser.from_str('p1:w p2:w')
        res: MatchResult = wp.match('hello world')
        self.assertTrue(res.success)
        self.assertEqual(res.results['p1'], 'hello')
        self.assertEqual(res.results['p2'], 'world')

    def test_match_2w(self):
        wp: WordParser = WordParser.from_str('p1:2w')
        res: MatchResult = wp.match('hello world')
        self.assertTrue(res.success)
    ...
    def test_match_1_n(self):
        wp: WordParser = WordParser.from_str('p1:n')
        res: MatchResult = wp.match('1234')
        self.assertTrue(res.success)
        self.assertEqual(res.results['p1'], 1234)

    def test_match_1_n_error(self):
        wp: WordParser = WordParser.from_str('p1:n')
        res: MatchResult = wp.match('not_a_number')
        self.assertFalse(res.success)

    def test_match_1_w_any_n(self):
        wp: WordParser = WordParser.from_str('word:w numbers:*n')
        res: MatchResult = wp.match('hello 1 2 3')
        self.assertTrue(res.success)
    ...
    def test_match_time_interval(self):
        wp: WordParser = WordParser.from_str('cmd:w duration:t msg:*w')
        res: MatchResult = wp.match('hello 1h30m a b c')
        self.assertTrue(res.success)
    ...
    def test_parse_time_interval(self):
        data: List[str] = ['1s', '5s', '1m30s', '1h20m5s', '1d8h30m10s']
        expected: List[float] = [1., 5., 90., 4805., 117010.]
